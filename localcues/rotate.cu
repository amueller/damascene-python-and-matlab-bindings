 ...
 */
void dispatchGradient(bool tg, int width, int height, int border, int nbins, float thetaPi, int rotatedWidth, int radius, bool blur, float sigma, int* devIntegrals, int integralImagePitchInInts, float* devGradientA, float* devGradientB) {
  ...
  //float effectiveRadius = M_SQRT2l*(-4.0+4.0*float(radius)*sqrtf(M_PIl))/8.0;
...
void dispatchGradient_64(int width, int height, int border, int nbins, float thetaPi, int rotatedWidth, int radius, bool blur, float sigma, int* devIntegrals, int integralImagePitchInInts, float* devGradientA, float* devGradientB) {
